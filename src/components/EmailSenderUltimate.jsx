  ...
  return (
    <div style={s.container}>
      ...
      <div style={{ maxWidth: 1600, margin: '0 auto' }}>
        ...
        {activeTab === 'logs' && <div style={s.card}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}><h2 style={{ fontSize: 20, margin: 0 }}>📋 Sending Logs & Errors</h2><div style={{ display: 'flex', gap: 8 }}><button onClick={() => setSendingLog([])} style={{ ...s.btn, ...s.btnDanger, ...s.btnSmall }}><Trash2 size={14}/> Clear All</button><button onClick={() => { const data = sendingLog.map(l => `[${l.timestamp}] ${l.type.toUpperCase()}: ${l.message}`).join('\n'); const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([data])); a.download = 'sending-logs.txt'; a.click(); }} style={{ ...s.btn, ...s.btnPrimary, ...s.btnSmall }}><Download size={14}/> Export</button></div></div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}><div style={s.statBox}><div style={{ fontSize: 22, fontWeight: 700, color: theme.accent }}>{sendingLog.length}</div><div style={{ fontSize: 12, color: theme.textMuted }}>Total Logs</div></div><div style={s.statBox}><div style={{ fontSize: 22, fontWeight: 700, color: theme.success }}>{sendingLog.filter(l => l.type === 'success').length}</div><div style={{ fontSize: 12, color: theme.textMuted }}>Success</div></div><div style={s.statBox}><div style={{ fontSize: 22, fontWeight: 700, color: theme.danger }}>{sendingLog.filter(l => l.type === 'error').length}</div><div style={{ fontSize: 12, color: theme.textMuted }}>Errors</div></div><div style={s.statBox}><div style={{ fontSize: 22, fontWeight: 700, color: theme.warning }}>{sendingLog.filter(l => l.type === 'warning').length}</div><div style={{ fontSize: 12, color: theme.textMuted }}>Warnings</div></div></div><div style={{ background: '#0a0a0f', borderRadius: 12, border: '2px solid ' + theme.accent, overflow: 'hidden' }}><div style={{ background: theme.accentGradient, padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><h3 style={{ margin: 0, color: '#fff', fontSize: 14, fontWeight: 700 }}>📊 Detailed Log Entries</h3><span style={{ fontSize: 12, color: 'rgba(255,255,255,0.8)' }}>{sendingLog.length} entries</span></div><div style={{ maxHeight: 600, overflowY: 'auto', padding: 0 }}>{sendingLog.length > 0 ? sendingLog.map((log, i) => <div key={i} style={{ padding: '14px 20px', borderBottom: '1px solid ' + theme.cardBorder, display: 'flex', gap: 12, alignItems: 'flex-start' }}><div style={{ minWidth: 70, fontSize: 11, color: theme.textMuted, fontFamily: 'monospace' }}>{log.timestamp}</div><div style={{ flex: 1 }}><div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}><span style={{ ...s.badge, background: log.type === 'success' ? theme.success + '25' : log.type === 'error' ? theme.danger + '25' : log.type === 'warning' ? theme.warning + '25' : theme.accent + '25', color: log.type === 'success' ? theme.success : log.type === 'error' ? theme.danger : log.type === 'warning' ? theme.warning : theme.accent, padding: '3px 10px', fontSize: 11 }}>{log.type.toUpperCase()}</span>{log.smtp && <span style={{ fontSize: 11, color: theme.textMuted, background: theme.input, padding: '3px 8px', borderRadius: 4 }}>SMTP: {log.smtp}</span>}</div><div style={{ fontSize: 13, color: theme.text, wordBreak: 'break-word' }}>{log.message}</div></div></div>) : <div style={{ padding: 60, textAlign: 'center', color: theme.textMuted }}><AlertCircle size={60} style={{ opacity: 0.3, marginBottom: 16 }}/><p>No logs yet. Send emails to see logs here.</p></div>}</div></div><div style={{ marginTop: 24, background: theme.accent + '15', borderRadius: 12, padding: 20, border: '2px solid ' + theme.accent + '30' }}><h3 style={{ margin: '0 0 12px', fontSize: 14, color: theme.accent }}>📖 Log Legend</h3><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 }}><div><span style={{ ...s.badge, background: theme.success + '25', color: theme.success }}>SUCCESS</span><p style={{ margin: '8px 0 0', fontSize: 13, color: theme.textMuted }}>Email delivered successfully</p></div><div><span style={{ ...s.badge, background: theme.danger + '25', color: theme.danger }}>ERROR</span><p style={{ margin: '8px 0 0', fontSize: 13, color: theme.textMuted }}>Failed to send (SMTP error, auth fail, etc)</p></div><div><span style={{ ...s.badge, background: theme.warning + '25', color: theme.warning }}>WARNING</span><p style={{ margin: '8px 0 0', fontSize: 13, color: theme.textMuted }}>Campaign paused, aborted, or delayed</p></div><div><span style={{ ...s.badge, background: theme.accent + '25', color: theme.accent }}>INFO</span><p style={{ margin: '8px 0 0', fontSize: 13, color: theme.textMuted }}>Campaign started, resumed, waiting</p></div></div></div></div>}
      </div>

      {showSmtpModal && <div style={s.modal} onClick={() => setShowSmtpModal(false)}><div style={s.modalContent} onClick={e => e.stopPropagation()}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}><h2 style={{ margin: 0 }}>{editingSmtp ? 'Edit' : 'Add'} SMTP Account</h2><button onClick={() => setShowSmtpModal(false)} style={s.btnIcon}><X size={24}/></button></div><div><label style={{ fontSize: 13, color: theme.textMuted }}>Account Name *</label><input style={s.input} placeholder="e.g., My Gmail" value={newSmtp.name} onChange={e => setNewSmtp(p => ({ ...p, name: e.target.value }))}/></div><div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 12 }}><div><label style={{ fontSize: 13, color: theme.textMuted }}>SMTP Host *</label><input style={s.input} placeholder="smtp.gmail.com" value={newSmtp.host} onChange={e => setNewSmtp(p => ({ ...p, host: e.target.value }))}/></div><div><label style={{ fontSize: 13, color: theme.textMuted }}>Port *</label><input style={s.input} placeholder="587" value={newSmtp.port} onChange={e => setNewSmtp(p => ({ ...p, port: e.target.value }))}/></div></div><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}><div><label style={{ fontSize: 13, color: theme.textMuted }}>Username *</label><input style={s.input} placeholder="your@email.com" value={newSmtp.username} onChange={e => setNewSmtp(p => ({ ...p, username: e.target.value }))}/></div><div><label style={{ fontSize: 13, color: theme.textMuted }}>Password *</label><input type="password" style={s.input} placeholder="password" value={newSmtp.password} onChange={e => setNewSmtp(p => ({ ...p, password: e.target.value }))}/></div></div><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}><div><label style={{ fontSize: 13, color: theme.textMuted }}>From Name</label><input style={s.input} placeholder="Your Name" value={newSmtp.fromName} onChange={e => setNewSmtp(p => ({ ...p, fromName: e.target.value }))}/></div><div><label style={{ fontSize: 13, color: theme.textMuted }}>From Email *</label><input style={s.input} placeholder="you@domain.com" value={newSmtp.fromEmail} onChange={e => setNewSmtp(p => ({ ...p, fromEmail: e.target.value }))}/></div></div><div style={{ background: theme.accent + '15', borderRadius: 12, padding: 16, border: '2px solid ' + theme.accent + '30', marginBottom: 16 }}><h3 style={{ margin: '0 0 16px', fontSize: 14, color: theme.accent }}>🔐 Encryption Settings</h3><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}><div><label style={{ fontSize: 13, color: theme.textMuted }}>Encryption Type *</label><select style={{ ...s.input, marginBottom: 0 }} value={newSmtp.encryption} onChange={e => setNewSmtp(p => ({ ...p, encryption: e.target.value }))}><option value="STARTTLS">STARTTLS (Port 587)</option><option value="SSL/TLS">SSL/TLS (Port 465)</option><option value="None">None (Port 25)</option></select></div><div style={{ display: 'flex', alignItems: 'flex-end' }}><label style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 13, fontWeight: 600, cursor: 'pointer', padding: '10px 0' }}><input type="checkbox" checked={newSmtp.secure} onChange={e => setNewSmtp(p => ({ ...p, secure: e.target.checked }))} style={{ width: 20, height: 20 }}/><span style={{ color: theme.text }}>Secure Connection</span></label></div></div><div style={{ fontSize: 11, color: theme.textMuted, marginTop: 12, padding: 10, background: theme.input, borderRadius: 8 }}>💡 <strong>Port 465:</strong> Use SSL/TLS + Secure ON | <strong>Port 587:</strong> Use STARTTLS + Secure OFF | <strong>Port 25:</strong> Use None + Secure OFF</div></div><button onClick={addSmtpAccount} style={{ ...s.btn, ...s.btnPrimary, width: '100%', marginTop: 12, justifyContent: 'center' }}>{editingSmtp ? 'Update Account' : 'Add Account'}</button></div></div>}
